...
class EducationSpending(CodingProblem):
    ...
    def check(self, query, results):
        ...
        assert (results.country_name.iloc[0] == 'Cuba'), ('Cuba should be the top value, but you have {}'.format(results.country_name.iloc[0]))
...
class FindInterestingCodes(CodingProblem):
    ...
    def check(self, query, results):
        ...
        lower_query = query.lower()
        assert ('num_rows' in results.columns)
    ...
    _hint = "The part before `FROM` is `SELECT indicator_code, indicator_name, COUNT(*) num_rows`"
    _solution = CS(\
"""
code_count_query = \"""SELECT indicator_code, indicator_name, COUNT(*) num_rows
