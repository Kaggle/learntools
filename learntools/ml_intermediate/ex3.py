...
class LabelB(CodingProblem):
    _vars = ['ordinal_X_train', 'ordinal_X_valid']
...
""")
    ...
    def check(self, ordinal_X_train, ordinal_X_valid):

        assert type(ordinal_X_train) == pd.core.frame.DataFrame, \
        "`ordinal_X_train` is not a pandas DataFrame."

        assert type(ordinal_X_valid) == pd.core.frame.DataFrame, \
        "`ordinal_X_valid` is not a pandas DataFrame."

        assert not(any((ordinal_X_train.dtypes == 'object').values)), \
        "You still need to encode some of the categorical columns in your training data."

        assert not(any((ordinal_X_valid.dtypes == 'object').values)), \
        "You still need to encode some of the categorical columns in your validation data."
        
        # remove 45 after nb update
        assert ordinal_X_train.shape[1] in [57, 45], \
        "`ordinal_X_train` does not have the correct number of columns."

        # remove 45 after nb update
        assert ordinal_X_valid.shape[1] in [57, 45], \
        "`ordinal_X_valid` does not have the correct number of columns."
...
class CardinalityB(EqualityCheckProblem):
    _vars = ['OH_entries_added', 'ordinal_entries_added']
    _expected = [990000, 0]
